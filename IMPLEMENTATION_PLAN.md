...
## Priority 5: GitHub Pages Deployment

- Spec says serve from root of `master` branch with `index.html` at root, but file structure places source in `src/`
- Recommended approach: add a root `index.html` that redirects to `src/index.html` (simplest, no GitHub Pages config needed)
- Alternative: configure GitHub Pages to serve from `src/` folder via repository settings
- This is a deployment config choice, not a spec change
